...
#include <iostream>
#include "tic tac toe methods temp.cpp"
...
int main () {
    ...
    current.game::get_UC();                 //gets player symbols
    ...
    int l = 1;
        ...
        current.setplayer_C(current.game::symbol_C());
        cout << "\ncurrent player is: " << current.getplayer_C() << endl;              // show current player symbol
        current.game::select_UI();                 //gets player spot selection, and checks if spot is already taken
        ...
        current.game::print_CB();                                           // takes selection and marks the board
